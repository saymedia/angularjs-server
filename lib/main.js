...
function flattenPromises($q, obj) {
    ...
    var flattenObj = function (obj) {
        // Since we're modifying the object as we go we have to capture its keys
        // ahead of time and iterate over that list.
    ...
    var watchForAllPromises = function () {
        $q.all(promises).then(
            function () {
                if (promises.length > 0) {
                    // wait for more promises, but do it asynchronously
                    // so we don't smash the stack if we do lots of
                    // iterations.
                    setTimeout(
                        function () {
                            watchForAllPromises();
        ...
        // empty promises out for the next iteration.
        promises = [];
    };
