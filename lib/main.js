...
function makeRequestListener(options) {
    ...
    var staticResourceDir = options.staticResourceDir;

    var initShim = [];
    initShim.push('<script type="text/javascript">window.stop();setTimeout(function () {');
        ...
        var onceRouteKnown = function (route) {
            ...
            $httpBackend.notifyWhenNoOpenRequests(
                function () {
                    ...
                    response.end(initShim + staticSnapshot);
                    context.dispose();
                }
            );
        };
